process R_PRS_QC {
    ...
    tuple path(bed), path(bim), path(fam), path (prune), path (het), path(HCM_GWAS_QC), val(condition)
    
    

    output:
    tuple path(bed), path(bim), path(fam), path(HCM_GWAS_QC), path ("*_het_valid_out_vs_HCM_GWAS*.sample"), path("*_a1_cohort_bim_vs_HCM_GWAS*"), path("*_mismatching_SNPs_vs_HCM_GWAS*"), val(condition),  emit: QC_het_a1_mismatch

    
    script:
    """
    R_PRS_QC2.R ${het} ${bim} ${HCM_GWAS_QC} ${condition}
